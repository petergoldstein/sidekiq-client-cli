...
describe SidekiqClientCLI do
  ...
  describe "ARGV parsing" do
    ...
    it "parses push with classes" do
      ...
      expect(client.settings.config_path).to eq SidekiqClientCLI::DEFAULT_CONFIG_PATH
      expect(client.settings.queue).to eq nil
      expect(client.settings.retry).to eq nil
    end
    ...
    it "parses push with a queue" do
      ...
      expect(client.settings.command_args).to eq worker_klasses
      expect(client.settings.config_path).to eq SidekiqClientCLI::DEFAULT_CONFIG_PATH
      expect(client.settings.queue).to eq "my_queue"
    end
    ...
    it 'parses push with a boolean retry' do
      ...
      expect(client.settings.command_args).to eq worker_klasses
      expect(client.settings.config_path).to eq SidekiqClientCLI::DEFAULT_CONFIG_PATH
      expect(client.settings.retry).to eq false
    end
    ...
    it 'parses push with an integer retry' do
      ...
      expect(client.settings.command_args).to eq worker_klasses
      expect(client.settings.config_path).to eq SidekiqClientCLI::DEFAULT_CONFIG_PATH
      expect(client.settings.retry).to eq 42
    end
  ...
  describe "run" do
    ...
    it "loads the config file if existing and runs the command" do
      ...
      expect(File).to receive(:exists?).with(config_path).and_return true
    ...
    it "won't load a non-existant config file and the command is run" do
      ...
      expect(File).to receive(:exists?).with(config_path).and_return false
  ...
  describe 'push' do
    ...
    it 'returns false if at least one #push_argument call fails' do
      expect(client).to receive(:push_argument).with('FirstWorker').and_return(true)
      expect(client).to receive(:push_argument).with('SecondWorker').and_return(false)
  ...
  describe '#push_argument' do
    ...
    it "pushes the worker classes" do
      expect(Sidekiq::Client).to receive(:push).with('class' => klass1,
                                                 'args' => [],
                                                 'queue' => default_queue,
    ...
    it "prints and continues if an exception is raised" do
      ...
      expect(out).to include("Failed to push")
    end
